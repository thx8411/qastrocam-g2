...
\title{Manuel d'utilisation Qastrocam-g2 vers. 4.9}
...
\item vers. 4.9
\begin{itemize}
\item work on philips cams support over kernel 3 started
\item qwtplot3d no more needed
\item V4L1 only supported on kernel 2
\item missing icons segfault bug fixed
\item QHY6 support added (imager and telescope)
\item QHY5's bands filter added
\item the sound alert can be disabled
\item lost star alarm added in guiding module
\item some guiding module bugs fixed
\item Ra and Dec can be inverted on telescope keypad
\item the ghost module can now be paused
\item telescope simulator added
\item simulation camera added
\item settings camera chooser changed
\item settings file version changed
\item guiding settings now stored
\item generic LX200 telescope added
...
CCD philips). Les pilotes V4L2 sont support\'es depuis la version 4.5. Il est \'a noter que
le noyau linux 3 ne supporte plus les pr\'eriph\'riques V4L1. Certaines fonctionnalit\'es de
l'ancien pilote pwc n'ayant pas encore \'et\'e transpos\'ees dans le pilote V4L2, les cameras
philips ou compatibles ne pourrons \^etre exploit\'ees pleinement qu'avec un noyau 2.
...
\paragraph*{}
Pour compiler qastrocam-g2, il vous faut d'abord g\'en\'erer les Makefile,
 avec la commande make configure-plateforme. La commande make vous permet 
alors de g\'en\'erer le binaire. Pour g\'en\'erer un package debian, utilisez
la commande fakeroot ./debian/rules binary en tant qu'administrateur. Le package 
debhelper est alors n\'ec\'essaire. Pour g\'en\'erer cette documentation, 
vous avez besoin du package texlive.
